...
const exampleJsonFiles = [
  ...
  { name: "Predicted Route Example", path: "/data/askcos_route_sample.json", askcosRoute: true },
  { name: "Hybrid Routes Example", path: "/data/hybrid_routes_example.json" },
];

const ExampleGraphs = () => {
  const { setAicpGraph, appSettings } = useContext(MainContext);
      ...
      <Flex align="flex-start" gap="small">
        {exampleJsonFiles.map((example) => (
          <div key={"load_div_" + example.name}>
            <Button onClick={() => handleLoadExample(example.path, example.askcosRoute || false)}>
              {example.name}
            </Button>
            <br />
          </div>
