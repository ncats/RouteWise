...
#
# If a substance is on the left hand-side of the reaction equation but none of its atoms are found in the product, it's
# a reagent otherwise it's a reactant.
#
# REMARK
#
# This code is adapted from the original code written by Gergely. This code can be found in the 'references' folder at
# the root of the repository. Additionally some code was adapted as part of 'decomposition_utils'.
#
...
def determine_max_reactant_index_from_parsed_rxn(
    parsed_rxn: decomposition_utils.ParsedReaction,
) -> int:
...
def determine_min_product_index_from_parsed_rxn(
    parsed_rxn: decomposition_utils.ParsedReaction,
) -> int:
...
def determine_max_product_index_from_parsed_rxn(
    parsed_rxn: decomposition_utils.ParsedReaction,
) -> int:
    return determine_max_product_index(parsed_rxn.rxsmiles)


def are_fragment_indices_coherent(
    fragment_block, max_reactant_index, min_product_index, max_product_index
):
    first = True
    ...
    for fragment in fragment_block:
        if int(fragment) <= int(max_reactant_index) and int(fragment) >= int(0):
            index_type = "reactant"
        elif int(fragment) >= int(min_product_index) and int(fragment) <= int(
            max_product_index
        ):
            index_type = "product"
        elif int(fragment) > int(max_reactant_index) and int(fragment) < int(
            min_product_index
        ):
            index_type = "reagent"
        else:
...
def validate_f_section_indices(rxsmiles: str) -> bool:
    ...
    if not parsed_rxn.fragment_groups:
        raise ValueError(
            f"[ERROR] No fragment groups found in the parsed RXSMILES: {rxsmiles}"
        )

    # Only these are defined as the reagents are either present of not
    # Always true: min_reactant_index = 0    # zero-indexed
    max_reactant_index = determine_max_reactant_index_from_parsed_rxn(
        parsed_rxn
    )  # zero-indexed
    min_product_index = determine_min_product_index_from_parsed_rxn(
        parsed_rxn
    )  # zero-indexed
    max_product_index = determine_max_product_index_from_parsed_rxn(
        parsed_rxn
    )  # zero-indexed

    for fragment_block in parsed_rxn.fragment_groups:
        if not are_fragment_indices_coherent(
            fragment_block, max_reactant_index, min_product_index, max_product_index
        ):
            raise decomposition_utils.FragmentGroupError(
                "[ ERROR] Role section border indexing error. Please check the code logic."
            )
    ...
    for fragment_block in parsed_rxn.fragment_groups:
        for fragment_index in fragment_block:
            if fragment_index not in unique_fragment_indices.keys():
                ...
                raise decomposition_utils.FragmentGroupError(
                    f"[ERROR] Not all original fragment indices are unique in the extension. Please check RXSMILES. Skipping RXSMILES {rxsmiles}, {parsed_rxn.fragment_groups}."  # noqa: E501
                )
...
def get_charge(smiles):
    ...
    if mol is None:
        raise ValueError(
            f"[ERROR] SMILES not parseable in the get_charge function. Please check the SMILES {smiles}. Skipping RXSMILES."  # noqa: E501
        )
    return Chem.GetFormalCharge(mol)


def reconstruct_rxsmiles(substances: List[dict[str, Any]]) -> str:  # noqa: C901
    reactants = []
    reagents = []
    products = []
    ...
    reactants_sorted = sorted(
        reactants, key=lambda x: x["heavy_atom_count"], reverse=True
    )
    reagents_sorted = sorted(
        reagents, key=lambda x: x["heavy_atom_count"], reverse=True
    )
    products_sorted = sorted(
        products, key=lambda x: x["heavy_atom_count"], reverse=True
    )
    ...
    for substance in reactants_sorted:
        ...
        if len(fragments) > 1:
            fragment = ".".join(
                str(i) for i in range(total_index, total_index + len(fragments))
            )
...
def normalize_roles(rxsmiles: str) -> str:
    ...
    if not rxsmiles_has_atommapping(rxsmiles):
        raise RxsmilesAtomMappingException(
            rxsmiles=rxsmiles,
            message=f"No atom mapping found in the RXSMILES: {rxsmiles}.",
        )
    ...
    if parsed_rxn.fragment_groups and len(parsed_rxn.fragment_groups) > 0:
        has_fragment_extension = True  # noqa: F841
        validate_f_section_indices(rxsmiles)

    rxsmiles_no_extension = parsed_rxn.rxsmiles_no_extension  # noqa: F841
    fragments_groups = parsed_rxn.fragment_groups  # noqa: F841
    ...
    all_substances: List[dict[str, Any]] = (
        [
            {"smiles": reactant, "role": "reactant", "og_role_index": i}
            for i, reactant in enumerate(reactants)
        ]
        + [
            {"smiles": reagent, "role": "reagent", "og_role_index": i}
            for i, reagent in enumerate(reagents)
        ]
        + [
            {"smiles": product, "role": "product", "og_role_index": i}
    ...
    for substance in all_substances:
        ...
        if "." in substance["smiles"]:
            ...
            # This helps to identify the individual reagent substances, despite RDKit does not seem to separate them
            # above the arrow by a plus sign like in the case of reactants and products.
...
def _compute_rbi(parsed_reaction: decomposition_utils.ParsedReaction) -> float:
    ...
    for substance in parsed_reaction.reactants:
        ...
        if mol:
            atoms = set([atom for atom in mol.GetAtoms()])  # noqa: C403,C416
            ...
            all_reactant_atom_indices = all_reactant_atom_indices.union(
                reactant_atom_indices
            )
    ...
    for substance in parsed_reaction.products:
        ...
        if mol:
            ...
            all_product_atom_indices = all_product_atom_indices.union(
                product_atom_indices
            )
...
def _compute_tbi(parsed_reaction: decomposition_utils.ParsedReaction) -> float:
    ...
    for substance in parsed_reaction.products:
        ...
        if mol:
            ...
            all_product_atom_indeces = all_product_atom_indeces.union(
                product_atom_indices
            )
    ...
    # factor of 2 incorporated that matching atom indices are only accounted for only one time up to this point due to
    # the intersection operation
    tbi = float(100.0 * float(len(matching_indices))/float(len(all_atoms)-len(matching_indices)))
    return tbi
