...
from decomposition_utils import parse_reaction_smiles
...
def remove_piped_text(input_string):
    """
    Remove text between pipes at the end of a string.
    """
    # OLD method: return re.sub(r'\|\S*\|$', '', input_string)
...
def template_smarts_to_image(smarts, svg=True, transparent=True, return_png=True, retro=True, dummy_atoms=True, highlight=True, plus=True, **kwargs):
    """
    ...
    for i in range(rxn.GetNumReactantTemplates()):
        ...
        if plus and i > 0:
            images.append(draw_plus(svg=svg, transparent=transparent))
        images.append(mol_to_image(mol, svg=svg, transparent=transparent, clear_map=False, update=True, **kwargs))
